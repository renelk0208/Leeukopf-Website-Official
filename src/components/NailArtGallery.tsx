...
/** 
 * Configuration for nail art product subcategories.
 */
const NAIL_ART_CATEGORIES = [
  { 
    id: '3dGel', 
    folder: '3D-multifunctional-gel', 
    title: '3D Multifunctional Gel',
    description: 'Versatile 3D art gels for creative designs'
  },
  { 
    id: 'mirrorPowders', 
    folder: 'Solid Mirror Powders', 
    title: 'Chrome & Mirror Powders',
    description: 'Ultra-fine pigments for mirror finishes'
  },
];

/**
 * Use Vite's import.meta.glob to dynamically load all images from nail art folders.
 */
...
function buildCategoryImages(): Record<string, { src: string; alt: string }[]> {
  ...
  NAIL_ART_CATEGORIES.forEach((category) => {
  ...
  Object.keys(imageModules).forEach((path) => {
    ...
    // Skip the category image in the root nail-art folder
    if (path.includes('nail-art-category')) return;

    // Extract the folder name from the path
    const pathParts = path.split('/');
    const nailArtIndex = pathParts.findIndex((part) => part === 'nail-art');
    if (nailArtIndex === -1 || nailArtIndex + 1 >= pathParts.length) return;

    const folderName = pathParts[nailArtIndex + 1];
    const filename = pathParts[pathParts.length - 1];

    // Find the matching category
    const category = NAIL_ART_CATEGORIES.find((cat) => cat.folder === folderName);
    if (!category) return;
    ...
    <div className="mb-10 sm:mb-12 md:mb-16">
      <div className="text-center mb-6 sm:mb-8">
        <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2 sm:mb-3">
          Nail Art Product Gallery
        </h3>
        <p className="text-base sm:text-lg text-gray-600 font-light max-w-2xl mx-auto px-2">
          Click on any category below to browse our nail art products.
        </p>
      </div>
