...
const BUILDER_SUBCATEGORIES = [
  ...
  { 
    ...
    description: 'One product to build, shape and finish'
  },
  { 
...
/**
 * Use Vite's import.meta.glob to dynamically load all images from builder systems folders.
 */
const imageModules = import.meta.glob<{ default: string }>(
  '/public/img/products/builder-systems/**/*.jpg',
  { eager: true }
);
...
function buildCategoryImages(): Record<string, { src: string; alt: string }[]> {
  ...
  // Initialize empty arrays for each subcategory
  BUILDER_SUBCATEGORIES.forEach((category) => {
  ...
  Object.keys(imageModules).forEach((path) => {
    ...
    const builderSystemsIndex = pathParts.findIndex((part) => part === 'builder-systems');
    if (builderSystemsIndex === -1 || builderSystemsIndex + 1 >= pathParts.length) return;

    const folderName = pathParts[builderSystemsIndex + 1];
...
/** Main component: displays subcategory cards that open a gallery modal */
export default function BuilderSystemsGallery() {
    ...
    <div className="mb-10 sm:mb-12 md:mb-16">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <p className="text-base sm:text-lg text-gray-600 font-light max-w-2xl mx-auto px-2">
          Click on any category below to browse our complete range of builder gel products.
        </p>
      </div>
